...
EchoMinutes 1.1.1 支持系统音频与麦克风采集、实时中英文转写、离线媒体导入、说话人分离、历史会话、转写编辑和多格式导出。音频与转写默认保存在本机；应用本身不上传会议内容，模型由用户安装后按需下载。
...
8. 检查并按需编辑片段、重命名或合并说话人后，点击“导出”。已保存记录可从“历史会话”重新打开。
...
维护者只需推送形如 `v1.1.1` 的 tag，GitHub Actions 会完成构建、测试、无模型权重检查，并创建 Windows x64 安装器、便携 ZIP、对应 SHA256 和自动更新说明：
...
- 历史会话目前保存转写与 speaker 信息，尚未把录音文件路径写入数据库，因此还不能从时间戳直接跳转播放。
...
Core、SQLite、导出、模型下载、STT 接口与 CLI 调度保持 `net8.0`，当前 85 项核心测试已在 Windows 11 和 Ubuntu 24.04 实际通过；CI 还会在 macOS runner 上验证。WPF、WASAPI、Windows 更新器和仓库携带的运行时二进制仍绑定 Windows，不能把核心测试通过解读为完整桌面支持。
