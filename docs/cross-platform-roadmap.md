...
EchoMinutes 1.1 的核心数据、导出、模型目录、下载器、STT 接口和外部 CLI 调度均为 `net8.0`。这些项目及 85 项核心测试已在 Windows 11 与 Ubuntu 24.04 / `linux-x64` 上实际通过；CI 还会在 `windows-latest`、`ubuntu-latest`、`macos-latest` 上重复验证。
...
1. 为历史会话保存录音轨道路径，才能实现时间轴点击播放和缺失文件诊断。
